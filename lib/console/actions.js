...
// FIXME: promise returned by rpc call, since reject callback is never called.
...
class Actions {
  ...
  loadGraph(command, callback) {
    ...
    this._driver.getGraph(g).then(msg => {
      if (msg._payload.code == 0) {
        ...
        process.stdout.write("\nGraph [" + command.graphName + "] loaded.\n");
      }
    });
  ...
  createGraph(command, callback) {
    ...
    this._driver.createGraph(g).then((msg) => {
      if (msg._payload.code == 0)
        process.stdout.write("Graph [" + command.graphName + " created.\n");
      else
        process.stdout.write("An error occurred while trying to create the graph.\n");
    });
    ...
    if (command.entity == 'vertex') {
      ...
      try {
        ...
        this._driver.createVertex(v).then(msg => {
          if (msg._payload.code == 0)
            process.stdout.write("Vertex done to graph [" + graphid + "].\n");
          else
            process.stdout.write("An error occurred while trying to add the vertex to the graph [" + graphid + "].\n");
        });
      ...
      try {
        ...
        this._driver.createEdge(e).then(msg => {
          if (msg._payload.code == 0)
            process.stdout.write("Edge added to graph [" + graphid + "].\n");
          else
            process.stdout.write("An error occurred while trying to add the edge to the graph [" + graphid + "].\n");
        });
  ...
  del(command, callback) {
    ...
    if (command.entity == 'vertex') {
      ...
      this._driver.deleteVertex(v).then(msg => {
        if (msg._payload.code == 0)
          process.stdout.write("Vertex deleted from graph [" + graphid + "].\n");
        else
          process.stdout.write("An error occurred while trying to delete the vertex from the graph [" + graphid + "]\n");
      });
      ...
      /* rpc call */
      let e = new Edge({from : fid, to : tid, graphid : graphid});
      this._driver.deleteEdge(e).then(msg => {
        if (msg._payload.code == 0)
          process.stdout.write("Edge delete from graph [" + graphid + "].\n");
        else
          process.stdout.write("An error occurr while trying to delete the vertex from the graph [" + graphid + "]\n");
      });
  ...
  listGraph(command, callback) {
    ...
    if (command.options.vertice) {
      ...
      let vid = (command.options.vertice === true) ? null : command.options.vertice;
      let v = new Vertex({id: vid, graphid: graphid});
      this._driver.getVertexList(v).then(list => {

        console.log("\nResults: " + list.length + " vertices (graph [" + graphid +").\n");
      ...
      this._driver.getEdgeList(e).then(list => {
        ...
        console.log("\nResults: " + list.length + " edges (graph [" + graphid + "].\n");
        for (var i = 0; i < list.length; i++) {
          let e = list[i];
          console.log("{");
    ...
    if (command.entity == 'vertex') {
      ...
      try {
        ...
        this._driver.updateVertex(vertex).then(msg => {
          if (msg._payload.code == 0)
            process.stdout.write("Vertex updated from graph [" + graphid +"].\n");
          else
            process.stdout.write("An error occurred.\n");
        });
      ...
      try {
        ...
        this._driver.updateEdge(edge).then(msg => {
          if (msg._payload.code == 0)
            process.stdout.write("Edge updated from graph [" + graphid + "].\n");
          else
            process.stdout.write("An error occurred.\n");
        });
