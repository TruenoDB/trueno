...
class InternalAPI {
  ...
  init() {
    ...
    return new Promise((resolve, reject)=> {
      ...
      if (self._clusterHosts && self._clusterHosts[thisHost].master) {
        /* Logging the message */
        ...
        self.__internalConnection.connect((id)=> {
          ...
          self.__internalConnection.sentReadySignal({name: thisHost, settings: self._clusterHosts[thisHost]})
          .then((response)=> {
            /* log process */
            logger.info(">> CLUSTER: Master proceed signal received...");
            /* master response to continue, resolve the promise right away */
            resolve(thisHost + ':' + self._port);
          });
        }, ()=> {
  ...
  in_hostReadySignal(reply, msg) {
    ...
    if (currentTotal == total) {
      ...
      /* Master continues */
      this._cluserReadyResolve(thisHost + ':' + self._port);
    }
  }
