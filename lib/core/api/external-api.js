...
class ExternalAPI {
  ...
  init() {
    ...
    /* Instantiate Backing Storage Maneger */
  ...
  ex_fetch(reply, msg) {
    ...
    self._search.fetch(pld.ftr, pld.graph, pld.type).then((results)=> {

      /* After getting the indexes, fetching the data from storage backend (C*) */
      self._storage.fetch(results, pld.graph, pld.type).then((list)=> {
        /* Success */
        logger.info('ex_fetch. [', list,  ']');
        logger.info('ex_fetch. [', pld.type, '] Successful! ');
      }, (err) => {
        /* Error */
        logger.info('ex_fetch. [', err, ']');
        logger.info('ex_fetch. [', pld.type, '] Failure!');
      });
  ...
  ex_destroy(reply, msg) {
    ...
    /* Destroying object in the storage backend (C*) */
    self._storage.destroy(pld.obj, pld.graph, pld.type).then((results)=> {
      /* Success */
      logger.info('ex_destroy. [', pld.type, '] Successful! ');
    }, (err) => {
      /* Error */
      logger.info('ex_destroy. [', pld.type, '] Failure!');
    });

    /* Destroying indexes in elastic search */
    self._search.destroy(pld.obj, pld.ftr, pld.graph, pld.type).then((results)=> {
  ...
  ex_persist(reply, msg) {
    ...
    console.log('ex_create --> ', pld);

    /* Persisting in the storage backend (C*) */
    self._storage.persist(pld.obj, pld.graph, pld.type).then((results)=> {
      /* Success */
    ...
    self._search.persist(pld.obj, pld.graph, pld.type).then((results)=> {
      let rsp = Message.buildMessage({status: Status.response.SUCCESS, payload: results});
      reply(rsp);
    }, (error)=> {
  ...
  ex_create(reply, msg) {
    ...
    self._storage.createGraph(pld.obj, pld.graph).then((results)=> {
      logger.info('ex_create: graph successfully created -> ', results);
    }, (error)=> {
      logger.info('ex_create: error while creating graph -> ', error);
    });

    /* Persisting in elastic search */
    self._search.createGraph(pld.obj, pld.graph, pld.type).then((results)=> {
      let rsp = Message.buildMessage({status: Status.response.SUCCESS, payload: results});
      reply(rsp);
    }, (error)=> {
      let rsp = Message.buildMessage({status: Status.response.ERROR, payload: error});
