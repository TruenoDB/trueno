...
class ExternalAPI {
  ...
  ex_bulk(reply, msg) {
    ...
    promises.push(self._storage.bulk(pld.operations, pld.graph));
    /* Persisting in elastic search */
    promises.push(self._search.bulk(pld.operations, pld.graph));
  ...
  ex_compute(reply, msg) {
    ...
    let objComputeScheduler = new ComputeScheduler({parent: self});

    /* Expected Object from Driver
      Job = {
          algorithmType: "PageRank"
          graph:         "schema in the cassandra backend",
          TOL:           "Tolerance",
          alpha:         "Reset Probability"
    };
    */
    ...
    objComputeScheduler._ex_compute(pld.job).then((jobId) => {
