...
class ExternalAPI {
  ...
  init() {
    ...
    self._compute = new ComputeScheduler({parent: self});
  ...
  ex_open(reply, msg) {
    ...
    self.ex_fetch(function(result) {
      if (result._status == Status.response.ERROR) {
        /* if the index was not found, let's create it on the backend */
        if (result._payload.type == 'index_not_found_exception') {
          self.ex_create(reply, msg);
        } else {
          logger.error('Fetching Data from Index error', result._payload);
        }
      } else {
        reply(result);
      }
  ...
  ex_bulk(reply, msg) {
    ...
    while (backendBuffer.length) {
      ...
      promises.push(self._storage.bulk(backendBuffer.splice(0, 100), pld.graph));
    }
    /* Insert operations in batches of 300(found optimal)*/
    while (indexBuffer.length) {
      /* Persisting in elastic search */
      promises.push(self._search.bulk(indexBuffer.splice(0, 300), pld.graph));
    }
  ...
  /* FIXME. Most probably all the function below will have to either: (1) delete; (2) adapt; since most of the API was
     replaced with above functions. */
