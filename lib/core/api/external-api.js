...
class ExternalAPI {
  ...
  ex_persist(reply, msg) {
    ...
    /* Persisting in backing storage (C*) */
  ...
  ex_create(reply, msg) {
    ...
    // self._storage.createGraph(pld.obj).then((results)=> {
  ...
  ex_bulk(reply, msg) {
    ...
    /* Persisting in elastic search */
    this._search.bulk(pld.operations,pld.graph).then((results)=> {
      let rsp = Message.buildMessage({status: Status.response.SUCCESS, payload: results});
      reply(rsp);
    }, (error)=> {
      let rsp = Message.buildMessage({status: Status.response.ERROR, payload: error});
    ...
    /* logging */
    logger.info('ex_bulk invoked', pld);
  }
