...
class ExternalAPI {
  ...
  /**
   * Persists component changes.
   * @param {function} reply - Reply function for the request(send back to the client).
   * @param {object} msg - The requested job object.
   */
  ex_persist(reply, msg) {
    ...
    /** Logging */
    logger.debug('ex_persist invoked', e);

    /** Search */
    let search = new SearchManager({index : e.obj.label});

    /** Build message */
    msg = Message.buildMessage({payload: {code: 0, result: 'persisted'}});

    /** Reply to the client */
    reply(msg);

    // TODO
    // Test search class using Id to create index
    // Insert in Cassandra and use ID to create index
    console.log("ex_persist ");
    console.log(e);
  }//ex_persist
