...
class ExternalAPI {
  ...
  ex_destroy(reply, msg) {
    ...
    this._search.destroy(pld.obj, pld.ftr, pld.graph, pld.type).then((results)=> {
  ...
  ex_persist(reply, msg) {
    ...
    /* Persisting in elastic search */
    this._search.persist(pld.obj, pld.graph, pld.type).then((results)=> {
      let rsp = Message.buildMessage({status: Status.response.SUCCESS, payload: results});
      reply(rsp);
    }, (error)=> {
  ...
  ex_create(reply, msg) {
    ...
    /* Persisting in elastic search */
    this._search.createGraph(pld.obj, pld.graph, pld.type).then((results)=> {
      let rsp = Message.buildMessage({status: Status.response.SUCCESS, payload: results});
      reply(rsp);
    }, (error)=> {
      let rsp = Message.buildMessage({status: Status.response.ERROR, payload: error});
