...
class ExternalDriver {
  ...
  getVertex(v) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._rpc.call('ex_getVertex', msg).then(msg => {

        if (msg._payload.code == 0) {

          let vertex = new Vertex;
          if (msg._payload.result) {
            utils.datatoComponent(msg._payload.result, vertex);
          }
          resolve(vertex);
        }
  ...
  getEdge(e) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._rpc.call('ex_getEdge', msg).then(msg => {

        if (msg._payload.code == 0) {

          let edge = new Edge;
          if (msg._payload.result) {
            utils.datatoComponent(msg._payload.result, edge);
          }
          resolve(edge);
        }
