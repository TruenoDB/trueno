...
class ComponentDownloader {
  ...
  download(cmp, resolve) {
    ...
    var strm = Download(url, filePath, {extract: url.endsWith(".zip")});

    /* When download is complete, fire this event */
    strm.then(()=> {
      /* If file is not zip, uncompress with tar */
      if(!url.endsWith(".zip")){
        shell.exec("tar xf " + filePath+'/'+url.split('/').pop() + " -C " + filePath, {async: false});
      }
    })
    .then(()=> {
      /* Normalize directory */
