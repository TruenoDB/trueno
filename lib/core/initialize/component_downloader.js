...
class ComponentDownloader {
  ...
  start() {
    ...
    /* Start all downloads */
    this.download(this._components.shift());
  }
  ...
  download(cmp) {
    ...
    var strm = Download(url, __dirname + '/../binaries/' + compObj.dir, {extract: true});
    ...
    strm.then(()=> {
      ...
      if (self._components.length > 0) {
        self.download(self._components.shift());
      } else {
        logger.info('All components downloaded!, now execute: $ trueno start');
      }
    }, (err)=> {
