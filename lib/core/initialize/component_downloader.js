...
class ComponentDownloader {
  ...
  download(cmp, resolve) {
    ...
    /* filePath */
    var filePath = path.normalize(__dirname + '/../binaries/' + compObj.dir);
    /* Starting the download and bind functions */
    var strm = Download(url, filePath, {extract: false});
    ...
    strm.then(()=> {
      ...
      /* Uncompress with zip */
      if(url.endsWith('zip')){
        res = shell.exec("unzip -qq " + filePath + '/' + url.split('/').pop() + " -d " + filePath, {async: false});
      }else{
        /* Uncompress with tar */
        res = shell.exec("tar xf " + filePath + '/' + url.split('/').pop() + " -C " + filePath, {async: false});
      }

      /* verifying if compression was successful */
      if(res.stderr){
    ...
    .then(()=> {
      ...
      logger.info('Failed to download or decompress ' + compObj.name + ' from ' + url);
      logger.info(err);
      ...
      if (compObj.links.length > 0) {
        ...
        logger.error('Unable to download or decompress the' + compObj.name + ' component, setup is will halt...');
      }
    });
