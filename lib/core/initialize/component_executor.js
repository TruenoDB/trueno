...
class ComponentExecutor {
    ...
    return new Promise((resolve, reject)=> {
      ...
      pm2.connect(function (err) {
        ...
        pm2.launchBus(function(err, bus) {
          logger.info('🔌 SubProcess bus connected');
          bus.on('process:event', function(data) {
            if(data.event ==='exit'){
              logger.info('⚠️', data.process.name, 'exited!');
            }
          });
        });
      });
  ...
  run(compObj, resolve, reject) {
    ...
    let proc = {
      ...
      silent: false,
      cwd: componentPath,
      output: process.dataBasePath + '/logs/' + compObj.dir + '-out.log',
      error: process.dataBasePath + '/logs/' + compObj.dir + '-err.log'
    };
    /* start process */
    pm2.start(proc, function (err, apps) {
      if (err) throw err
      /* logging component starting */
      logger.info('⏳ ' + compObj.name + ' starting...');
      /* the suppress error flag */
