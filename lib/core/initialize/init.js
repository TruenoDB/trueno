...
const GraphManager = require('../../backend/trueno-storage-manager');
...
class Init {
  ...
  constructor(param = {}) {
    ...
    /* Component Executer */
    this._executer = new ComponentExecuter();
  }

  initConfig(file) {
    // TODO: Set provided config file if any
    global.config = YAML.load(__dirname + '/../../../conf/trueno-single.yaml');
  }
  ...
  setRunningState(s) {
    /* Change running status */
    this._executer.setRunningState(s);
  }
  ...
  start() {
    ...
    return new Promise((resolve, reject)=> {
      ...
      self._extAPI.init().then((addrs)=> {
        ...
        logger.info('ElasticSeach engine connected at: ' + addrs[1]);
