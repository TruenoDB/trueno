  ...
  _initConfig(param) {
    ...
    if (param.clusterHosts) {
      ...
      if (param.clusterHosts[param.host]) {
        /* deploy backend */
        Components.cassandra.deploy = param.clusterHosts[param.host].backend;
        /* deploy index */
        Components.elasticsearch.deploy = param.clusterHosts[param.host].index;
        /* deploy compute server */
        Components.truenoComputeServer.deploy = param.clusterHosts[param.host].master;
        /* deploy compute master */
        Components.sparkMaster.deploy = param.clusterHosts[param.host].master;
        /* deploy compute worker */
        Components.sparkWorker.deploy = (param.clusterHosts[param.host].compute && !param.clusterHosts[param.host].master);
      } else {
      ...
      for (let host in param.clusterHosts) {
        ...
        if (param.clusterHosts[host].master) {
          ...
          Components.sparkWorker.cmdFlags[1] = host;
          break;
        }
      }
    }
    ...
    Components.sparkWorker.cmdFlags[5] = path.normalize(dataBasePath + Components.sparkWorker.cmdFlags[5]);
    ...
    if (!fs.existsSync(Components.sparkWorker.cmdFlags[5])) {
      shell.mkdir('-p', Components.sparkWorker.cmdFlags[5]);
    }
  }
