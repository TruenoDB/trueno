...
class Init {
  ...
  _initConfig(param) {
    ...
    thisHost = thisHost || 'localhost'; //require('os').hostname();
    /* setting new host */
    global.config.local.host = thisHost;
    ...
    if (param.clusterHosts) {
      ...
      if (param.clusterHosts[thisHost]) {
        /* deploy index */
        Components.elasticsearch.deploy = param.clusterHosts[thisHost].index && Components.elasticsearch.deploy;
        /* deploy compute server */
        Components.truenoComputeServer.deploy = param.clusterHosts[thisHost].master && Components.truenoComputeServer.deploy;
        /* deploy compute master */
        Components.sparkMaster.deploy = param.clusterHosts[thisHost].master && Components.sparkMaster.deploy;
        /* deploy compute worker */
        Components.sparkWorker.deploy = (param.clusterHosts[thisHost].compute && !param.clusterHosts[thisHost].master) && Components.sparkWorker.deploy;
      } else {
    ...
    /* setting the bridge server shards and replicas */
    let shards = Number(Components.truenoElasticBridgeServer.numberOfShards);
    let numOfMachines = (param.clusterHosts)?Object.keys(param.clusterHosts).length:1;
    let replicas = Math.ceil(Number(Components.truenoElasticBridgeServer.replicaPercent) * numOfMachines);
    Components.truenoElasticBridgeServer.cmdFlags[2] = shards;
    Components.truenoElasticBridgeServer.cmdFlags[3] = replicas;
