...
class Init {
  ...
  _initConfig(param) {
    ...
    /* set process database path */
    process.dataBasePath = dataBasePath;
  ...
  verifyAndSanitizeComponents() {
    ...
    /* first remove binaries/cerebro/RUNNING_PID if present */
    shell.rm('-f', __dirname + '/../binaries/cerebro/RUNNING_PID');
    /* clear elasticsearch configuration */
    shell.exec('> '+__dirname + '/../binaries/elasticsearch/config/elasticsearch.yml');
  ...
  start() {
    ...
    return new Promise((resolve, reject)=> {
      ...
      self._intAPI.init().then((addrs)=> {
        ...
        /* Successfully started Internal api */
        logger.info('Cluster status collected. ', status._instance._metrics);
