...
class SearchManager {
  ...
  _buildCoordinatedSearchQuery(id, ftr, sFtr, dir, idx, type){
    ...
    if(dir === 'out') {
      ...
      console.log(JSON.stringify(builtFilter));

      filter.query.filtered.query = builtFilter.query;
      /* Setting the source */
      filter.query.filtered.filter.filterjoin.id = {
        "indices": [
          idx
        ],
        "types": [
      ...
      console.log('in');
      let builtFilter2 = self.buildFilter(ftr).build();
      console.log(JSON.stringify(builtFilter2));

      filter.query.filtered.query = builtFilter2.query;
      /* Setting the source */
      filter.query.filtered.filter.filterjoin.id = {
        "indices": [
          idx
