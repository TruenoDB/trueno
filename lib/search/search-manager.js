...
class SearchManager {
  ...
  constructor(param = {}) {
    ...
    this._neighbors = require('./neighbors.json');
    this._out = require('./out.json');
   ...
   * @returns {new Coordinated Search Query}
   * @private
   */
  _buildCoordinatedSearchQuery(id, ftr, sFtr, dir, idx, type){
    ...
    let filter = self._out;
    /* Building client's filter */
    let builtFilter = self.buildFilter(ftr).build();

    if(dir === Enums.dirEdge.OUTGOING) {

      /* Assigning client's filter, validate when ftr is null [MATCH_ALL]*/
      if(!ftr) {
        filter.query.filtered.query = builtFilter.query;
      }
      /* Setting the index */
      filter.query.filtered.filter.filterjoin.id.indices[0] = idx;
      /* Setting the foreign key */
      filter.query.filtered.filter.filterjoin.id.path = Enums.edgeEndPoint.TARGET;
      /* Setting the value of vertex.id as first index filter */
      filter.query.filtered.filter.filterjoin.id.query.term.source = id;

    }
    else {

      /* Assigning client's filter */
      filter.query.filtered.query = builtFilter.query;
      /* Setting the index */
      filter.query.filtered.filter.filterjoin.id.indices[0] = idx;
      /* Setting the foreign key */
      filter.query.filtered.filter.filterjoin.id.path = Enums.edgeEndPoint.SOURCE;
      /* Setting the value of vertex.id as first index filter - all e with target id */
      filter.query.filtered.filter.filterjoin.id.query.term = {"target":id};
    }
