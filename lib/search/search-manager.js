...
class SearchManager {
  ...
  constructor(param = {}) {
    ...
    /* utilities */
    this._utils = {};
    /* the edge filter by id */
    this._utils.edgeFtr = {"query": {"filtered": {"filter": {"term": {"id": null}}}}};
    /* the edge's vertices filter */
    this._utils.edgeVerticesFtr = {"query": {"filtered": {"filter": {"bool": {"should": [{"term": {"id": null}}, {"term": {"id": null}}]}}}}};
  ...
  /**
   * Count requested components to client.
   * @return {promise} The promise with the results.
   */
  count(ftr, idx, type) {
  ...
  vertices(id, idx) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let filter = self._utils.edgeFtr;
      ...
      self._search.search(filter, idx, 'e').then((edges) => {
        ...
        if (edges.length > 0) {
          ...
          let eFilter = self._utils.edgeVerticesFtr;
          eFilter.query.filtered.filter.bool.should[0].term.id = edges[0]._source.source;
          eFilter.query.filtered.filter.bool.should[0].term.id = edges[0]._source.target;
