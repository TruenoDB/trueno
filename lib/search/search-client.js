...
class Search {
    ...
    return new Promise((resolve, reject)=> {
      ...
      self._client.ping({
        ...
        if (error) {
          ...
          // TOFIX. Replace the socket address once an stable version of the elastic bridge server is integrated
          // self._nativeClient = Socket('http://localhost:'+global.config.components.truenoElasticBridgeServer.cmdFlags[1]
          self._nativeClient = Socket('http://localhost:8008'
            ,connectionOptions);
          /* connection handler */
          self._nativeClient.on('connect', function(){
  ...
  bulk(b, index) {
    ...
    return new Promise((resolve, reject)=> {
      ...
      self._nativeClient.emit('bulk', payload, function(results) {
          //console.log(results);
  ...
  search(f, index, type, filterPrebuilded) {
    ...
    return new Promise((resolve, reject)=> {
      ...
      //var hrstart = process.hrtime();
      self._nativeClient.emit('search', payload, function(results) {
          //console.log(results);
          //var hrend = process.hrtime(hrstart);
          //console.info("Execution time: %dms", hrend[1]/1000000);
          if(results==null){
