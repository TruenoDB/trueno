...
class Search {
  ...
  constructor(param = {}) {
    ...
    /* native client */
    this._nativeClients =  [];
    /* native client roulete */
    this._nativeRoulette = 0;
  ...
  init() {
    ...
    return new Promise((resolve, reject)=> {
      self._client = new elasticsearch.Client({
        hosts: global.config.connections.index
      });
      ...
      self._client.ping({
        ...
        if (error) {
          ...
          self._nativeClients[0] = Socket('http://localhost:'+global.config.components.truenoElasticBridgeServer01.cmdFlags[1]
            ,connectionOptions);
          /* connection handler */
          self._nativeClients[0].on('connect', function(){
              /* connecting native client */
              self._nativeClients[1] = Socket('http://localhost:'+global.config.components.truenoElasticBridgeServer02.cmdFlags[1]
                ,connectionOptions);
              /* connection handler */
              self._nativeClients[1].on('connect', function(){
                  /* Resolve promise */
                  resolve(global.config.connections.index);
              });
          });
        }
      });
  ...
  bulk(b, index) {
    ...
    return new Promise((resolve, reject)=> {
      ...
      self._nativeClients[self._nativeRoulette].emit('bulk', payload, function(results) {
          //console.log(results);
      ...
      /* increment the native roulette */
      self._nativeRoulette = (self._nativeRoulette++) % self._nativeClients.length;
  ...
  /**
   * Delete a document with the provided id.
   * @param {component} f - The document to be deleted, must contain id, type, and index.
   * @return {promise} The promise with the results.
   */
  ...
  search(f, index, type, filterPrebuilded) {
    ...
    return new Promise((resolve, reject)=> {
      ...
      /* Building the query */
      let q = self._buildNativeQueryFilter(f, index, type, filterPrebuilded);

      /* if query is wrapped, clean for native query structure */
      // if(q.query){
      //   q = q.query.filtered.filter
      // }
      ...
      //var hrstart = process.hrtime();
      self._nativeClients[self._nativeRoulette].emit('search', payload, function(results) {
          //var hrend = process.hrtime(hrstart);
          //console.info("Execution time: %dms", hrend[1]/1000000);
          if(results==null){
            resolve([]);
   ...
   * @return {string} The query string for the elasticsearch engine.
   */
  _buildNativeQueryFilter(f, index, type, prebuilded) {
    /* build the query */
    let obj = (prebuilded) ? f : f.build();
    /* search by term */
    return obj;
  }
