    ...
    request.post(strRequest)
        ...
        .end(function(err, res){
            if (err || !res.ok) {
                ...
                $('#tbl_jobs > tbody:last-child').append('<tr><td>'
                            + res.body.result.jobId + '</td><td>Page Rank</td><td>Started</td></tr>');
    ...
    request.post(strRequest)
        ...
        .end(function(err, res){
            if (err || !res.ok) {
                ...
                self._requestedJobs.push(res.body.result.jobId);
                $('#tbl_jobs > tbody:last-child').append('<tr><td>' + res.body.result.jobId
                              + '</td><td>Connected Components</td><td>Started</td></tr>');
    ...
    request.post(strRequest)
        ...
        .end(function(err, res){
            if (err || !res.ok) {
                ...
                self._requestedJobs.push(res.body.result.jobId);
                $('#tbl_jobs > tbody:last-child').append('<tr><td>' + res.body.result.jobId
                             + '</td><td>Strongly Connected Components</td><td>Started</td></tr>');
 ...
 *
 */
SparkJobClient.prototype.triangleCounting = function(graph) {

    var self = this;
    counter = 1;

    var strRequest = self._createHTTPRequestString(Enums.algorithmType.TRIANGLE_COUNTING);
    ...
    request.post(strRequest)
      ...
      .end(function(err, res){
        if (err || !res.ok) {
          ...
          $('#tbl_jobs > tbody:last-child').append('<tr><td>' + res.body.result.jobId
            + '</td><td>Strongly Connected Components</td><td>Started</td></tr>');
          self.setupTimer(res.body.result.jobId, Enums.algorithmType.TRIANGLE_COUNTING);
        }
      });
...
SparkJobClient.prototype.wordCountRequest = function() {
    ...
    var strRequest = self._createHTTPRequestString(Enums.algorithmType.WORD_COUNT);
    ...
    request.post(strRequest)
      ...
      .end(function(err, res){
        if (err || !res.ok) {
          ...
          self._requestedJobs.push(res.body.result.jobId);
          $('#tbl_jobs > tbody:last-child').append('<tr><td>' + res.body.result.jobId
            + '</td><td>Word Count</td><td>Started</td></tr>');
          self.setupTimer(res.body.result.jobId, Enums.algorithmType.WORD_COUNT);
        }
      });
 ...
 */
SparkJobClient.prototype.shortestPaths = function(graph, landmarks) {

  var self = this;
  counter = 1;

  var strRequest = self._createHTTPRequestString(Enums.algorithmType.SHORTEST_PATHS);
  ...
  request.post(strRequest)
    ...
    .end(function(err, res){
      if (err || !res.ok) {
        ...
        $('#tbl_jobs > tbody:last-child').append('<tr><td>' + res.body.result.jobId
          + '</td><td>Word Count</td><td>Started</td></tr>');
        self.setupTimer(res.body.result.jobId, Enums.algorithmType.SHORTEST_PATHS);
      }
    });
        ...
        request
            ...
            .end(function(err, res){
                ...
                if(data.status === Enums.jobStatus.FINISHED){
                    ...
                    $('#tbl_jobs > tbody:last-child').append('<tr><td>' + jobId + '</td><td>' + algorithmType
                                + '</td><td>' + data.status + '(' + data.duration + ')</td></tr>');
                    ...
                    if(algorithmType === Enums.algorithmType.PAGE_RANK){
                        ...
                        for(var i = 0; i < result.length; i++){
                            $('#tbl_pr > tbody:last-child').append('<tr><td>' + result[i][0]
                                       + '</td><td>' + result[i][1] + '</td></tr>');
                        }
...
SparkJobClient.prototype._createHTTPRequestString =  function(algorithmType) {
    ...
    if(algorithmType === Enums.algorithmType.SHORTEST_PATHS){
      strRequest += config.algorithmsPublishedJob + "&classPath=" + config.sh;
    }
