...
function ComputeScheduler() {
  ...
  self._sparkJobServer = config.sparkJobServer.defaultSparkJobServer;

  /* Spark Job Server Port */
  // self._sparkJobServerPort =  options.defaultPort ||
  //                             config.sparkJobServer.defaultPort;
  self._sparkJobServerPort = config.sparkJobServer.defaultPort;

  self._cassandraHost = global.config.components.cassandra.configFlags.listen_address;

  /* Cassandra default port */
  self._cassandraPort = global.config.components.cassandra.configFlags.native_transport_port;
  ...
  self._objAlgorithms = new Algorithm();
...
ComputeScheduler.prototype._ex_compute = function(job) {
  ...
  //console.log('host ', self._cassandraHost, 'port ', self._cassandraPort);
  objJobParameters.host        = self._cassandraHost;
  objJobParameters.port        = self._cassandraPort;
    ...
    switch (job.algorithmType) {
      ...
      case Enums.algorithmType.PAGE_RANK:
        ...
        self._objAlgorithms.run(objJobParameters).then(function (jobId) {
          ...
          self._trackJobId(jobId, job.algorithmType, Enums.jobStatus.STARTED);
      ...
      /* Get new ShortestPaths instance */
...
ComputeScheduler.prototype._ex_computeJobResult = function(jobId) {
  ...
  /* Create new Promise with the spark job request (status), new algorithmType */
  return new Promise( function(resolve, reject) {
...
ComputeScheduler.prototype._trackJobId = function(jobId, algorithmType, status) {
  ...
  self._jobRequests[jobId].status = status;
  self._jobRequests[jobId].algorithmType  = algorithmType;
