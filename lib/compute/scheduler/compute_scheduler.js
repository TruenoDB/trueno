...
function ComputeScheduler(options) {
  ...
  self._sHost = options._sHost || "localhost";

  /* Cassandra default port */
  self._sPort = options._sPort || "9042";
...
ComputeScheduler.prototype._ex_compute = function(job) {
  ...
  objJobParameters.host        = self._sHost;
  objJobParameters.port        = self._sPort;
    ...
    switch (job.algorithmType) {
      ...
      case Enums.algorithmType.SHORTEST_PATHS:
        ...
        self._objAlgorithms.run(objJobParameters).then(function (jobId) {
          ...
          self._jobRequests[jobId] = {};
          self._jobRequests[jobId].algorithmType = job.algorithmType;
