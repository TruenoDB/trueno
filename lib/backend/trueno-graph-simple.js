...
class TruenoGraphSimple {
  ...
  getVertexList(vertex) {
    ...
    return new Promise((resolve, reject) => {
      ...
      self._conn.execute(q.build(Components.VERTEX, Operations.GETALL, {keyspace : self._name})).then( result => {
        //console.log('result => ', result);
  ...
  getEdgeList(edge) {
    ...
    return new Promise((resolve, reject) => {
      ...
      self._conn.execute(q.build(Components.EDGE, Operations.GETALL, {keyspace : self._name})).then( result => {
