...
class TruenoGraphSimple {
  ...
  getGraph(graph) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // FIXME: Since the table only has one record, no need to use a predicate (change when graph table is relocated).
      self._conn.execute(q.build(Components.GRAPH, Operations.GET, {keyspace : self._name})).then( result => {
