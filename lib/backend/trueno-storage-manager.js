...
/* FIXME: General code clean up of all the backend API. */
/* FIXME: Test console features. Remember that the API has been changed since the console was tested. */
/* FIXME: Create test case for integration */
...
class TruenoStorageManager {
  ...
  persist(obj, idx, type) {
    ...
    if (!Number.isInteger(obj.id) && type != 'g') {
      return Promise.reject('All vertices and edges must have an integer id');
    }
  ...
  bulk(b, idx) {
    ...
    return new Promise((resolve, reject) => {
      ...
      g.batch(operations).then((good) => {
          ...
          logger.debug(console.log(operations));
          reject(err);
        }
      )
    });
