...
Monitor.prototype.updateMetrics = function (metrics) {

  /* Updating Memory */
  this._scope.memmory.data[parseInt(metrics.rand * 100 % this._scope.memmory.data.length)] += metrics.rand * 1000;
  var totMem = this._scope.memmory.data[0] + this._scope.memmory.data[1];
  this._scope.usedSize = parseInt(scope.data[0]);
  this._scope.freeSize = parseInt(scope.data[1]);
  this._scope.used = parseInt((scope.data[0] / totMem) * 100);
  this._scope.free = parseInt((scope.data[1] / totMem) * 100);

  /* Updating CPU */
  this._scope.cpu.data[0].shift();
  this._scope.cpu.data[0].push(metrics.rand * 100);
};