...
function GraphLabCompute(options, graphLabResultsCtrl, graphLabVisCtrl, graphLabQueryCtrl) {

  this._scope = options.scope;
  /* Create local and remote computers */
  this._localComp = new LocalComputer(options,graphLabResultsCtrl, graphLabVisCtrl);
  this._remoteComp = new RemoteComputer(options, graphLabResultsCtrl, graphLabVisCtrl);
  this._dialog = options.dialog;

  /* the query controller */
  this._graphLabQueryCtrl = graphLabQueryCtrl;
      ...
      $scope.answer = function (answer) {
        if (answer) {
          switch (selectedIndex) {
            case 0:
              self._localComp.compute(algo, $scope.model);
              break;
            case 1:
              self._remoteComp.compute(self._graphLabQueryCtrl.getSelectedGraph(), algo, $scope.model);
              break;
          }
        }
        $mdDialog.hide();
