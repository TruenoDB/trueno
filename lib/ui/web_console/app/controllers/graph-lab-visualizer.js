...
GraphLabVisualizer.prototype.addEdge = function (e) {
  ...
  for (var k in e.properties) {
    if (e.properties[k].hasOwnProperty(0)) {
...
GraphLabVisualizer.prototype.onCreate = function (e) {

  var self = this;


  /* build graph from memory here */
  this.createDialog({edges:[],vertices:[]});

}

GraphLabVisualizer.prototype.createDialog = function (g) {
  ...
  self._dialog.show({
    controller: function ($scope, $mdDialog) {
      ...
      $scope.edges = g.edges.length;
      $scope.vertices = g.vertices.length;
