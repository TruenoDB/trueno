...
GraphLabVisualizer.prototype.addComponents = function (cmp) {
  ...
  var self = this;
  /* add each component */
  cmp.v.forEach(function (v, i, a) {
    if (!self._vis.graph.nodes(v.getId())) {
      self.addVertex(v, i, a);
    }
  });
  cmp.e.forEach(function (e, i, a) {
    /* If edge does not exist, and source -> target are present, add the edge */
    if (!self._vis.graph.edges(e.getId()) && (self._vis.graph.nodes(e.getSource()) && self._vis.graph.nodes(e.getTarget()))) {
      self.addEdge(e);
    }
  });
...
GraphLabVisualizer.prototype.addVertex = function (v, i, a) {
  ...
  if (v.getLabel() && !this._colorMap[v.getLabel()]) {
    this._colorMap[v.getLabel()] = this._getStringColor(v.getLabel());
  }else{
    v.setLabel("vertex");
    this._colorMap[v.getLabel()] = this._getStringColor(v.getLabel());
  }
  ...
  var node = {
    ...
    color: '#' + this._colorMap[v.getLabel()]
  };
...
GraphLabVisualizer.prototype.addEdge = function (e) {
  ...
  /* check if label is in color map*/
  if (e.getLabel() && !this._colorMap[e.getLabel()]) {
    this._colorMap[e.getLabel()] = this._getStringColor(e.getLabel());
  }else{
    e.setLabel("edge");
    this._colorMap[e.getLabel()] = this._getStringColor(e.getLabel());
  }
  ...
  this._vis.graph.nodes(e.getSource()).degree++;
  this._vis.graph.nodes(e.getTarget()).degree++;
