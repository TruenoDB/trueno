...
function GraphLabQuery(options) {
  ...
  Object.defineProperty(this, '_results', {
    get: function () {
      return options.wConsole._graphLabCtrl._graphLabResultsCtrl;
    }
  });
...
GraphLabQuery.prototype.executeQuery = function (qryString, automatic) {
  ...
  this._inConn.executeQuery(qry.getQuery()).then(function (res) {
    ...
    qry.setSuccess(true);
    qry.setGraph(self._scope.select.selectedGraph);
    qry.setResult(res);
    ...
    if(!automatic){
      ...
      /* adding results */
      self._results.addQueryResult(qry);
      /* set blinker */
      self._visualizer.getBlinkers().results = true;
      /* show toast */
