 ...
 * Get all nodes.
 */
LocalComputer.prototype.getNodes = function () {
 ...
 * Get all edges.
 */
LocalComputer.prototype.getEdges = function () {
  return this._g.edges(true);
}
...
LocalComputer.prototype.compute = function (algo, params) {
  ...
  var resObj = {
    ...
    graph: 'In Memory',
    local: true,
    vertices: this._g.nodes().length,
    edges: this._g.edges().length,
    label: algo.label,
    startTime: new Date(),
    json: {
      params: params,
