...
webConsole.prototype.init = function () {
  ...
  /* Get the App Module */
  var panel = angular.module("webConsole", ["chart.js", 'ngMaterial', 'ui.ace', 'md.data.table', 'jsonFormatter', 'blockUI', 'schemaForm']);
  /* Setting up theme */
  ...
  panel.controller("grapLabCtrl",
    function ($scope, JSONFormatterConfig, $mdDialog) {
  ...
  this._exConn.connect(function (s) {
    ...
    /* 1.) Deploy Job (get back jobId)
     2.) With jobId check status until you get status == FINISHED
     3.) When FINISHED request result
     */
    /*
     let parameters = {
     ...
     self._exConn._rpc.call('ex_compute', msg).then((jobId)=> {
     ...
     var x = setInterval(function () {
         self._exConn._rpc.call('ex_computeJobStatus', msg).then((status)=> {
              console.log('Job Status: ', status);
              if (status == 'FINISHED') {
                self._exConn._rpc.call('ex_computeJobResult', msg).then((result)=> {
    ...
    g.getCompute().getAlgorithms().then((algorithms)=> {
      /* set the algorithms */
