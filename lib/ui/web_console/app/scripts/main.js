...
webConsole.prototype.init = function () {
  ...
  this._inConn = new InternalConnection();
  this._exConn = new Trueno();

  /* temporal connection */
  window.exConn = this._exConn;
  ...
  panel.controller("kpofCtrl",
    function ($scope) {
      self._kpofCtrl = new Kpof({scope: $scope, wConsole: self});
      self._kpofCtrl.init();
      console.log('Kpof Controller Initialized');
    });
  /* Initialize graph lab controller */
  panel.controller("grapLabCtrl",
    function ($scope, JSONFormatterConfig) {
      self._graphLabCtrl = new GraphLab({scope: $scope, jsonFormatter: JSONFormatterConfig, wConsole: self});
